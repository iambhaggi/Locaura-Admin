...
const RidersOffline = () => {
  ...
  useEffect(() => {
    const fetchRiders = async () => {
      ...
      setError(null);
      try {
        const response = await ridersAPI.getAll({ limit: 1000, is_online: false });
        const mapped = (response?.data?.data || response?.data || []).map((rider) => ({
          ...
          last_active: rider.last_active_at ? new Date(rider.last_active_at).toLocaleString() : '-',
        ...
        setError('Failed to fetch offline riders');
      } finally {
        setLoading(false);
      }
    };
  ...
  // Filter only offline riders
  const riders = allRidersData.filter(r => r.is_online === false);
  ...
  const handleSaveNew = () => {
    if (addDialog.data?.rider_name && addDialog.data?.phone) {
      console.log('Add offline rider:', addDialog.data);
  ...
  const handleSaveEdit = () => {
    if (editDialog.data) {
      console.log('Edit offline rider:', editDialog.data);
  ...
  const stats = [
    { label: 'Offline Riders', value: riders.length, color: '#757575' },
    ...
    { label: 'Total Trips', value: riders.reduce((sum, r) => sum + Number(r.total_trips || 0), 0), color: '#2196f3' },
    { label: 'Total Earnings', value: '₹' + (riders.reduce((sum, r) => sum + Number(r.total_earnings || 0), 0) / 100000).toFixed(1), color: '#4caf50' },
  ];

  return (
    <Container sx={{ py: 4 }}>
      {loading && <Typography sx={{ mb: 2 }}>Loading offline riders...</Typography>}
      {error && <Typography color="error" sx={{ mb: 2 }}>{error}</Typography>}

      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        <Box>
          <Typography variant="h4" sx={{ fontWeight: 'bold' }}>Offline Riders</Typography>
          <Typography variant="body2" sx={{ color: '#666', mt: 0.5 }}>Riders currently offline and not available for orders</Typography>
        </Box>
              ...
              <TableBody>
                {filteredRiders.map((rider) => (
                  <TableRow key={rider._id} hover>
                    ...
                    <TableCell sx={{ color: '#999' }}>{rider.last_active}</TableCell>
                    <TableCell>
      ...
      <Dialog open={detailsDialog.open} onClose={() => setDetailsDialog({ open: false, data: null })} maxWidth="sm" fullWidth>
        <DialogTitle sx={{ fontWeight: 'bold' }}>Offline Rider Details</DialogTitle>
          ...
          {detailsDialog.data && (
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              ...
              <Box>
                ...
                <Chip label={detailsDialog.data.kyc_status} size="small" color={detailsDialog.data.kyc_status === 'verified' ? 'success' : 'warning'} sx={{ mt: 0.5 }} />
              </Box>
              <Box>
              ...
              <Box>
                <Typography variant="caption" sx={{ color: '#666' }}>Last Active</Typography>
                <Typography sx={{ color: '#999' }}>{detailsDialog.data.last_active}</Typography>
              </Box>
              <Box>
      ...
      <Dialog open={addDialog.open} onClose={() => setAddDialog({ open: false, data: null })} maxWidth="sm" fullWidth>
        <DialogTitle>Add Offline Rider</DialogTitle>
      ...
      <Dialog open={editDialog.open} onClose={() => setEditDialog({ open: false, data: null })} maxWidth="sm" fullWidth>
        <DialogTitle>Edit Offline Rider</DialogTitle>
        <DialogContent sx={{ pt: 2, display: 'flex', flexDirection: 'column', gap: 2 }}>
          {editDialog.data && (
            <>
      ...
      <DeleteConfirmDialog
        open={deleteDialog}
        title="Delete Rider"
        description="This will remove the offline rider from the system. You can undo within 10 seconds."
